...
- [x] 確認記録の形式を決める（予算事業ID・資料 URL・資料の年度・確認日・一致した点／しなかった点・状態〔説明あり／一部／未解決〕）。置き場所はリポジトリ内の ignore 済みディレクトリ → `.claude/checks-format.md`、`checks/`
- [x] 0 円事業の個別調査メモ（`.claude/zero-budget-research/`）を確認記録に畳み直す（192 件。説明あり 39〔うちシート内だけで説明 24〕／一部 49／未解決 104）
- [x] シート間で同じ予算年度の金額が変わった事業を兆候にする → 兆候「シート間の改訂」（`amount_revised`、`--revision-ratio` 既定 1%、2024→2025 で 70 事業）
