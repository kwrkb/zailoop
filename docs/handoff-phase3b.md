...
- ID が変わった継続事業の候補提示。2025 年度にしかない「前年度事業」50 件が対象。名称・所管・関連事業（`1-5`）の一致で候補を出し、**自動結合はしない**（VISION 11）。`1-5` は `rs.Sheet.Related` で既に読んでいて、関連性 `統合元`/`統合先`/`分割元`/`分割先` はシートに書かれた関係として使える。名称の一致だけで候補を出すのは推定にあたるので、VISION 14（事業間の関係はシートの記載だけで示す）との兼ね合いを着手前にユーザーと確認する
