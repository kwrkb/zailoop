...
func (d Dir) loadIndicators(id string, sheet *Sheet) error {
	columns := []string{
	...
	// A structured key avoids collisions when free text contains separators.
	byKey := make(map[[5]string]int)
	return d.scan("3-1", id, columns, func(r *csvRow) error {
		key := [5]string{
		...
		if !exists {
			index = len(sheet.Indicators)
			byKey[key] = index
			sheet.Indicators = append(sheet.Indicators, Indicator{
		...
		indicator := &sheet.Indicators[index]
